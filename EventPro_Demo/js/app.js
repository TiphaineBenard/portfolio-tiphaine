...
function rendreApplication() {
  ...
  // Sur mobile (≤720px) : 5 onglets visibles + bouton "···" pour le reste
  // Sur desktop : tous les onglets visibles
  const estMobile = window.innerWidth <= 960;
  const TABS_MAX = estMobile ? 5 : vuesAutorisees.length;
  const tabsVisibles = vuesAutorisees.length > TABS_MAX ? vuesAutorisees.slice(0, TABS_MAX) : vuesAutorisees;
  const tabsPlus = vuesAutorisees.length > TABS_MAX ? vuesAutorisees.slice(TABS_MAX) : [];
  const vueActiveEstDansPlus = tabsPlus.some(v => v.id === ETAT.vueActive);
  ...
  app.innerHTML = `
    ...
    ${banniereAlerte}
    ${tabsPlus.length > 0 ? `
    <div id="nav-plus-menu" style="display:none; position:fixed; bottom:calc(58px + env(safe-area-inset-bottom)); right:10px; background:var(--blanc); border-radius:12px; box-shadow:0 4px 24px rgba(42,38,36,0.22); z-index:300; min-width:190px; overflow:hidden; border:1px solid var(--gris-ligne);">
      ${tabsPlus.map(v => `
        <div class="nav-plus-item" data-vue="${v.id}" style="display:flex; align-items:center; gap:12px; padding:13px 16px; cursor:pointer; border-bottom:1px solid var(--gris-ligne); color:${ETAT.vueActive === v.id ? 'var(--bordeaux)' : 'var(--charbon)'}; background:${ETAT.vueActive === v.id ? 'var(--creme)' : 'var(--blanc)'}; font-weight:${ETAT.vueActive === v.id ? '600' : '400'}; font-size:0.9rem;">
          <span class="material-symbols-outlined" style="font-size:1.25rem; color:${ETAT.vueActive === v.id ? 'var(--bordeaux)' : 'var(--charbon-clair)'};">${v.icon}</span>
          ${v.label}
        </div>
      `).join("")}
    </div>
    ...
    <nav class="tabs-principal" style="display:flex; align-items:center;">
      <div style="display:flex; flex:1; overflow-x:hidden;">
        ${tabsVisibles.map(v => `
        ...
        `).join("")}
        ${tabsPlus.length > 0 ? `
          <div class="tab-item ${vueActiveEstDansPlus ? 'actif' : ''}" id="btn-nav-plus" style="flex:1;">
            <span class="tab-icone material-symbols-outlined" aria-hidden="true">more_horiz</span>
            <span class="tab-label-court">Plus</span>
          </div>
        ` : ''}
      </div>
      <button id="btn-icone-sauvegarde" title="${alerteSauvegarde ? 'Sauvegarder maintenant' : 'Données sauvegardées'}"
        style="margin-right:8px; background:none; border:none; cursor:pointer; font-size:1.1rem; opacity:${alerteSauvegarde ? '1' : '0.4'}; flex-shrink:0;">
        💾
      </button>
    </nav>
    <div class="titre-section-mobile">${vuesAutorisees.find(v => v.id === ETAT.vueActive)?.label || ''}</div>
  ...
  // Onglets principaux
  document.querySelectorAll(".tab-item[data-vue]").forEach(el => {
    el.addEventListener("click", () => naviguerVers(el.dataset.vue));
  });

  // Bouton "···" → ouvre/ferme le menu déroulant
  const btnPlus = document.getElementById("btn-nav-plus");
  const menuPlus = document.getElementById("nav-plus-menu");
  if (btnPlus && menuPlus) {
    btnPlus.addEventListener("click", (e) => {
      e.stopPropagation();
      const ouvert = menuPlus.style.display !== "none";
      menuPlus.style.display = ouvert ? "none" : "block";
      if (!ouvert) {
        setTimeout(() => {
          document.addEventListener("click", function fermerMenu() {
    ...
    document.querySelectorAll(".nav-plus-item").forEach(el => {
      el.addEventListener("click", () => {
        menuPlus.style.display = "none";
        naviguerVers(el.dataset.vue);
      });
    });
  }
...
// Recalcule la nav si la fenêtre change de taille (rotation, responsive)
let _resizeNavTimer;
window.addEventListener("resize", () => {
  clearTimeout(_resizeNavTimer);
  _resizeNavTimer = setTimeout(() => {
    if (ETAT.utilisateur && ETAT.utilisateur.role && ETAT.vueActive !== "connexion") {
      rendreApplication();
    }
  }, 200);
});
