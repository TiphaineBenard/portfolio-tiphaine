...
function rendreNouvelleCommande(conteneur) {
  ...
  conteneur.innerHTML = `
    <div class="carte" style="margin-bottom:18px;">
      <div class="carte-titre"><h2 style="margin:0;">Nouvelle commande</h2></div>
      <div class="grille-3">
        <div class="champ">
          <label>Magasin *</label>
      ...
      <div class="champ">
        ...
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          ${INSTRUCTIONS_PREPARATION.map(ins => `
...
function afficherGrilleProduits(produits, terme) {
  ...
  // Boutons farce : sélection exclusive par produit
  conteneur.querySelectorAll("[data-farce-produit]").forEach(btn => {
    btn.addEventListener("click", () => {
      const produitId = btn.dataset.farceProduit;
      const valeur = btn.dataset.farceValeur;
      if (!panierCourant[produitId]) panierCourant[produitId] = {};
      panierCourant[produitId].farce = valeur;
      // Mettre à jour le style des boutons de la zone
      document.querySelectorAll(`[data-farce-zone="${produitId}"] [data-farce-produit]`).forEach(b => {
        const actif = b.dataset.farceValeur === valeur;
...
function zoneFarceHTML(produit) {
  if (!produit.optionsFarce || produit.optionsFarce.length === 0) return "";
  const actuelle = (panierCourant[produit.id] && panierCourant[produit.id].farce) || produit.optionsFarce[0];
  return `
    <div class="zone-farce" data-farce-zone="${produit.id}" style="margin-top:6px; display:flex; flex-wrap:wrap; gap:5px; align-items:center;">
      <span style="font-size:0.78rem; color:var(--charbon-clair); font-weight:600; white-space:nowrap;">Farce :</span>
      ${produit.optionsFarce.map(opt => `
        <button type="button" data-farce-produit="${produit.id}" data-farce-valeur="${opt}"
...
function suppLigneFarceHTML(produit, idx, farceActuelle) {
  if (!produit.optionsFarce || produit.optionsFarce.length === 0) return "";
  var parts = [];
  parts.push('<div style="margin-top:6px; display:flex; flex-wrap:wrap; gap:5px; align-items:center;">');
  parts.push('<span style="font-size:0.78rem; color:var(--charbon-clair); font-weight:600; white-space:nowrap;">Farce :</span>');
  for (var fi = 0; fi < produit.optionsFarce.length; fi++) {
    var opt = produit.optionsFarce[fi];
    var actif = opt === farceActuelle;
    var border = actif ? "var(--bordeaux)" : "var(--gris-ligne)";
    var bg     = actif ? "var(--creme-fonce)" : "white";
    var fw     = actif ? "700" : "400";
    parts.push('<button type="button" class="supp-farce-btn"'
      + ' data-supp-produit="' + produit.id + '"'
      + ' data-supp-idx="' + idx + '"'
      + ' data-supp-farce="' + opt + '"'
      + ' style="padding:2px 10px; border-radius:20px; font-size:0.8rem; cursor:pointer; font-family:inherit;'
      + ' border:2px solid ' + border + '; background:' + bg + '; font-weight:' + fw + ';">'
      + opt + '</button>');
  }
  parts.push('</div>');
  return parts.join("");
...
function zoneMultiVarianteSuppHTML(produit) {
  ...
  for (var si = 0; si < lignesSupp.length; si++) {
    ...
    html += '</div>';
    html += suppLigneFarceHTML(produit, si, l.farce);
...
function attacherListeneursMultiVarianteSupp(conteneur) {
  conteneur.querySelectorAll(".supp-ajouter-btn").forEach(function(btn) {
    btn.addEventListener("click", function() {
      ...
      var farceDefaut = produit && produit.optionsFarce && produit.optionsFarce.length > 0 ? produit.optionsFarce[0] : "";
      panierCourant[produitId].lignesSupp.push({ poidsKg: 0, farce: farceDefaut, specification: "" });
  ...
  conteneur.querySelectorAll(".supp-farce-btn").forEach(function(btn) {
    btn.addEventListener("click", function() {
      var produitId = btn.dataset.suppProduit;
      var idx = parseInt(btn.dataset.suppIdx);
      var valeur = btn.dataset.suppFarce;
      if (!panierCourant[produitId] || !panierCourant[produitId].lignesSupp) return;
      panierCourant[produitId].lignesSupp[idx].farce = valeur;
      var ligne = btn.closest(".mv-supp-ligne");
      if (ligne) ligne.querySelectorAll(".supp-farce-btn").forEach(function(b) {
        var a = b.dataset.suppFarce === valeur;
...
function changerQuantitePanier(produitId, delta) {
  ...
  // Auto-sélectionner la première option de farce si pas encore choisie
  if (nouveau > 0 && !panierCourant[produitId].farce) {
    const produit = ETAT.catalogue.find(p => p.id === produitId);
    if (produit && produit.optionsFarce && produit.optionsFarce.length > 0) {
      panierCourant[produitId].farce = produit.optionsFarce[0];
    }
  }
